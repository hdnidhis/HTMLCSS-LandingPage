...
const normalize = (css) => {
	return css
		.replace(/#(\w+)/gi, (hex) => hex.toLowerCase())
		.replace(/\b0px/gi, "0")
...
const css = postcss([short]).process(normalize(source_css), {
	from: "css/main.css",
}).root;
...
const diff = (check, base, name) => {
	const changes = (check, base) => {
		...
		for (const key in check) {
			if (R.not(R.equals(check[key], base[key]))) {
				results[key] =
					R.is(Object, check[key]) && R.is(Object, base[key])
						? changes(check[key], base[key])
						: check[key];
			}
		}
		return results;
	...
	if (R.isEmpty(results)) {
		...
		let message =
			"It looks like your " + name + " rule is missing the following:\n";
		let selectorMessage =
			"It seems that you are missing a rule with a selector like ";
		for (let key in results) {
			if (key == "selector") {
				message =
					...
					"`" +
					results[key] +
					"`, or you are missing key declarations in that rule.";
			} else if (key !== "unique") {
				message += "\n- `" + key + ": " + results[key] + "`";
			}
		}
...
const getRule = (selector, name) => {
	...
	if (R.not(R.isEmpty(base))) {
		return {
			...
			message: "",
			class: base[0].class,
			selector: base[0].selector,
		};
	} else {
		return {
			assert: false,
			message:
				"It seems that you are missing a rule with a selector of " + name + ".",
		};
	}
};
...
module.exports = {
	...
	section: getRule("section", declarations.section.selector),
	flex: getRule(".flex", declarations.flex.selector),
	...
	headerNav: getRule(".header-nav", declarations.headerNav.selector),
	...
	accountLinks: getRule(".account a", declarations.accountLinks.selector),
	...
	heroH2: getRule(".hero h2", declarations.heroH2.selector),
	...
	bannerTextLastChild: diffAny(
		declarations.bannerTextLastChild,
		"`Banner Text Last Child`"
	),
	category: getRule(".category", declarations.category.selector),
	categoryImage: getRule(".category img", declarations.categoryImage.selector),
	categoryText: diffAny(declarations.categoryText, "`Category Text`"),
	categoryTextDiv: getRule(
		".category-text > div",
		declarations.categoryTextDiv.selector
	),
	categoryTextH3: getRule(
		".category-text h3",
		declarations.categoryTextH3.selector
	),
	fullBanner: diffAny(declarations.fullBanner, "`Full Banner`"),
	fullBannerBefore: diffAny(
		declarations.fullBannerBefore,
		"`Full Banner Before`"
	),
	fullBannerImage: diffAny(declarations.fullBannerImage, "`Full Banner Image`"),
	fullBannerSidebar: diffAny(
		declarations.fullBannerSidebar,
		"`Full Banner Sidebar`"
	),
	fullBannerSidebarH4: diffAny(
		declarations.fullBannerSidebarH4,
		"`Full Banner Sidebar H4`"
	),
	fullBannerSidebarH4After: diffAny(
		declarations.fullBannerSidebarH4After,
		"`Full Banner Sidebar H4 After`"
	),
	fullBannerSidebarFirst: diffAny(
		declarations.fullBannerSidebarFirst,
		"`Full Banner Sidebar First`"
	),
	fullBannerSidebarSecond: diffAny(
		declarations.fullBannerSidebarSecond,
		"`Full Banner Sidebar Second`"
	),
	fullBannerSidebarButton: diffAny(
		declarations.fullBannerSidebarButton,
	...
	footerLinksHover: diffAny(
		declarations.footerLinksHover,
		"`Footer Links Hover`"
	),
	socialLists: diffAny(declarations.socialLists, "`Social Lists`"),
	socialListsLinks: diffAny(
		declarations.socialListsLinks,
		"`Social Lists Links`"
	),
	...
	newsletterLinkHeader: diffAny(
		declarations.newsletterLinkHeader,
		"`Newsletter Link Header`"
	),
	newsletterLinkSubheader: diffAny(
		declarations.newsletterLinkSubheader,
		"`Newsletter Link Sub Header`"
	),
	newsletterInput: diffAny(declarations.newsletterInput, "`Newsletter Input`"),
	newsletterButton: diffAny(
		declarations.newsletterButton,
		"`Newsletter Button`"
	),
